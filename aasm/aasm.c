/* AASM - ARM assembler  Alpha+ version  J. Garside  UofM   10/1/07           */
/*
//  To do								@@@@@@@@
//  Symbol table lists
...
//		Proper shakedown testing
//		Macros
//		Conditional assembly
//	BUG?	. in defw (etc.) fetches origin of defw, not position of word
//
...
#define MAX_PASSES               10    /* No of reiterations before giving up */
...
#define SYM_USER_VARIABLE    0x1000             /* Assembler control variable */
...
#define LIST_LINE_LENGTH   100                            /* Total line length */
...
int          input_line(FILE*, char*, unsigned int);
int          parse_mnemonic_line(char*, sym_table*, sym_table*, sym_table*);
...
int          do_literal(instr_set, type_size, int*, int, unsigned int*);
...
int          get_variable(char*, unsigned int*, int*, int*, int*, sym_table*);
int          get_operator(char*, unsigned int*, int*, int*);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

sym_table   *sym_create_table(char*, unsigned int);
int          sym_delete_table(sym_table*, int);
...
int          cmp_next_non_space(char*, int*, int, char);
int          test_eol(char);
...
int          allow_error(unsigned int, int, int);
...
int          assembly_pointer_defined;
unsigned int entry_address;
int          entry_address_defined;
...
int          div_zero_this_pass;  /* Indicates /0 in pass, prevents code dump */
int          dump_code;         /* Allow output (FALSE on last pass if error) */
...
int if_stack[IF_STACK_SIZE + 1];               /* Used for nesting IF clauses */
int if_SP;
...
if (argc == 1)
  {
  printf("ARM assembler v0.25 (10/1/07)\n");
...
int input_line(FILE *file, char *buffer, unsigned int max)
{
int i;
char c;
...
int parse_mnemonic_line(char *line, sym_table *a_table, sym_table *t_table,
                                    sym_table *d_table)
{
int i, j, k, okay;
...
/*             returned value.                                                */
/* Returns:  0 if value can fit in a single MOV/MVN instruction               */
/*             ext_value contains the immediate field                         */
/*           1 if a load is required (data structures have been set up)       */
...
int do_literal(instr_set instr_type, type_size size, int *ext_value,
               int first_pass, unsigned int *pError)
{
unsigned int value;
int what, PC_inc;
literal_record *pTemp;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  void assemble_define(int size)
    ...
    int terminate, increment, i, escape;
    char delimiter, c;
      ...
      if (((line[position] == '"') || (line[position] == '\'')  /*  String    */
        ...
        while ((line[position] != delimiter) && !terminate)
          ...
          if (c != '\0')
            ...
            if (last_pass)
              ...
              byte_dump(assembly_pointer + increment, c, line, size);
              }
            increment = increment + size;          /* Always one address here */
            position++;
            }
          else
        ...
        if ((error_code == eval_okay)
          ...
          if ((error_code == eval_okay) && (last_pass))  /* Plant, ltl endian */
            byte_dump(assembly_pointer + increment, temp, line, size);

          if (!last_pass) error_code = eval_okay;        /* Pretend it's okay */
          increment += size;              /* Continue, even if missing values */
          }
        else
          terminate = TRUE;
        }
    ...
    assembly_pointer += increment;                   /* Add total size at end */
    return;
    }
          ...
          if ((error_code == eval_okay)
            ...
            switch (do_literal(ARM, size, &value, first_pass, &error_code))
              ...
              case 0:                              /* Will fit in instruction */
                ...
                if ((value & 0x80000000) == 0)                      /* => MOV */
                  op_code = op_code | 0x03A00000 | data_op_imm( value);
                else                                                /* => MVN */
                  op_code = op_code | 0x03E00000 | data_op_imm(~value);
                break;

              case 1:                               /* Needs full-blooded LDR */ 
...
/*       to be translated too.  Is this acceptable?  @@                       */
                ...
                if ((token & 0xF0000000) == 0xE0000000) /* Only unconditional */
                  ...
                  if ((value & 1) != 0)               /* Not halfword aligned */
                    ...
                    if (((value & 0xFE000000) == 0x00000000)     /* In range? */
                     || ((value & 0xFE000000) == 0xFE000000))
                      op_code = 0xFA000000
                              | ((value&0x03FFFFFFC) >> 2) | ((value&2) << 23);
                    else error_code = SYM_OORANGE_BRANCH;
                  }
        ...
        case 0x00060000:                                           /* STM/LDM */
          ...
          if (error_code == eval_okay)
            ...
            op_code |= parse_reg_list(0x0000FFFF);/* Get bitmask of registers */
                                   /* Can manipulate some `global' variables! */
        ...
        case 0x00080000:                                               /* ADR */
          ...
          int adr_loop(int value, int count)               /* Local procedure */
            {
            int i, fixed;
          ...
          if ((reg = get_reg(line, &position)) >= 0)
            ...
            if (cmp_next_non_space(line, &position, 0, ','))
              ...
              if (error_code == eval_okay)
                ...
                if (((token & 0x00008000) == 0)       /* Fixed length: ADR(n) */
                 || ((token & 0x00003000) != 0))        /* >1 word, but fixed */
                  adr_loop(x, (token >> 12) & 3);  /* Pass length to cruncher */
                else                                 /* Variable length: ADRL */
                  {
                  adr_loop(x, -1);              /* Crunch without restriction */
//  printf("ADRL: %08X  %d\n", x, extras/4 + 1);	// Last param. is length generated
// @@@ Should record this size in case it needs forcing, later @@@
                  }
                }
              }
            else
              ...
              if (cmp_next_non_space(line, &position, 0, ','))     /* Any more? */
                ...
                if (cmp_next_non_space(line, &position, 0, '#'))         /* (1) */
                  ...
                  if (error_code == eval_okay)
                    {
                    if ((value & ~0x1F) == 0)
                      op_code = (op_code << 11) | (value << 6) | temp;
                    else error_code = SYM_OORANGE;
                    }
                  }
...
/* Separated out so other functions (e.g. "ARM") can call it too.             */
  ...
  //printf("Hello?\n");
        ...
        if (elf_section_valid)         /* else reuse previous (unused) number */
          { elf_section++; elf_section_valid = FALSE; }
        }
      }
    ...
    if (error_code == eval_okay)                                 /* Still OK? */
      ...
      if (fill)                               /* Any label is at source point */
        assemble_redef_label(assembly_pointer,
                        ...
                        my_label, &error_code, pass_count, last_pass, line);
...
if (((token & 0xF8000000) != 0xF8000000) && (my_label->sort != NO_LABEL))
    ...
    assemble_redef_label(assembly_pointer, assembly_pointer_defined, my_label,
                        &error_code, pass_count, last_pass, line);
...
if (error_code == eval_okay)
 ...
 condition = (sym_find_label(name, variable_table) != NULL);	// Should be logical expression @@@
      ...
      case 0xF8000000:                                                 /* EQU */
        ...
        if (my_label->symbol != NULL)
          ...
          if (my_label->sort == LOCAL_LABEL)
            my_label->local->flags  |= SYM_REC_EQU_FLAG;  /* `type' indicator */
          else
            my_label->symbol->flags |= SYM_REC_EQU_FLAG;  /* `type' indicator */
          }
        else
      ...
      case 0xF8010000:                                                 /* ORG */
        ...
        if (elf_section_valid)
          { elf_section++; elf_section_valid = FALSE; }
                                       /* else reuse previous (unused) number */
        break;
...
void assemble_redef_label(unsigned int value, int defined, own_label *my_label,
                         unsigned int *error_code, int pass_count,
                         int last_pass, char *line)
{
...
int sym_delete_table(sym_table *old_table, int export)
{
int i, some_kept;
...
int get_variable(char *input, unsigned int *pos, int *value, int *unary,
                              int *bracket, sym_table *symbol_table)
{
int status, radix;
unsigned int ii;
      ...
      if (input[ii] == '\'')                              /* Character constant */
        ...
        if (input[ii] == '.')
          ...
          if (assembly_pointer_defined)
            { *value = assembly_pointer; status = eval_okay; }
...
int cmp_next_non_space(char *line, int *pPos, int offset, char character)
{
int result;                                                /* Boolean, really */
...
int alpha_numeric(char c)                                           /* Crude! */
{
...
int allow_error(unsigned int error_code, int first_pass, int last_pass)
{
